...
void gui_create_boss_healthbar(char* name, Entity* boss)
{
    ...
    gui_comp_copy_text(comp_name, name);
...
void gui_set_interactable(const char* desc, InteractableFuncPtr func_ptr, Map* map, MapNode* map_node)
{
    ...
    if (desc != NULL)
        gui_comp_copy_text(inter_comp, desc);
    else
...
static void inventory_toggle(GUIComp* comp)
{
    gui_comp_toggle_flag(comp, GUI_COMP_FLAG_VISIBLE);
    gui_comp_toggle_flag(comp, GUI_COMP_FLAG_CLICKABLE);
    gui_comp_toggle_flag(comp, GUI_COMP_FLAG_HOVERABLE);
}
...
static void video_mode_right_arrow_click(GUIComp* comp, i32 button, i32 action, i32 mods)
{
    ...
    if (button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_PRESS) {
        if (data->current_mode+1 < num_modes) {
            ...
            gui_comp_copy_text(comp->parent, mode);
        }
    }
}
...
static void load_video_settings(GUIComp* root)
{
    ...
    gui_comp_copy_text(video_mode, video_mode_text);
...
static void load_preset_options(GUIComp* root)
{
    ...
    gui_comp_copy_text(menu, menu_text);
...
static void load_preset_runs(GUIComp* root)
{
    ...
    gui_comp_copy_text(new_run, new_run_text);
...
static void load_preset_test(GUIComp* root)
{
    ...
    for (i32 halign = 0; halign <= 3; halign++) {
        for (i32 valign = 0; valign <= 3; valign++) {
            GUIComp* comp = gui_comp_create(20, 20, 20, 20);
            gui_comp_set_flag(comp, GUI_COMP_FLAG_HOVERABLE, true);
            comp->hover = test_hover;
            gui_comp_set_color(comp, 0, 255, 255, 255);
            gui_comp_set_align(comp, halign, valign);
