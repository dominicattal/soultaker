...
Textures are described in config/textures.json and assigned an id at runtime. Each texture's id can be queried at runtime using the name in the config file with `texture_get_id`. 
