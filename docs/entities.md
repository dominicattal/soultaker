...
Entity config files are stored in `config/entities` and assigned a unique id at runtime.

An entity can be give generic data as a void pointer. This can be manipulated in the entity's callback function.

Entities have stats that are used in damage calculations and updates. These are
- (Max) Health
- Speed
- Magic Resistance
- Armor
This is a subset of the stats the player has. Generic entities have no use for most of the player stats.

Entities can be afflicted with debuffs (Not implemented yet).
...
- `update` has the name of the procedure that should be called each frame. It is called after the general entity update processes, such as movement and collision.
