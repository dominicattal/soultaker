...
The player has an inventory. The inventory is where items are configured (see `items.md`). The player can have 1 helmet, 1 chestplate, 1 leggings, and 1 boots equipped. The player can have as many weapons as the Weapon Slot stat allows. Every item costs some amount of souls to equip and must not exceed the amount of Soul Power the player has.
